...
static LQRController angleLQR(0, 0, 0, 0, 0); // Will be properly initialized in setup
bool lqr_initialized = false;
...
void balance()
{
  ...
  // Add safety check before using angleLQR
  if (!lqr_initialized) {
      return;
  }
  ...
  int16_t angle_control = angleLQR.calculate(ref_distance, ref_speed, ref_angle, ref_angleRate,
...
void initializeLQRController() {
  ...
  const int16_t angle_output_limit = -1;   // <= 0 is no limit

  // Initialize the static LQR controller
  angleLQR = LQRController(kx_scaled, kx_dot_scaled, kth_scaled, kth_dot_scaled, angle_output_limit);
  lqr_initialized = true;
}

void resetLQRControllers() {
  if (lqr_initialized) angleLQR.reset();
}
